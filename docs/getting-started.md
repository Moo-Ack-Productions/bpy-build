...
> [!IMPORTANT]
> For security and compatibility reasons, BpyBuild only allows a subset of characters
  for `addon_folder` and `build_name`. These are:
...
> [!CAUTION]
> Note: the addon source code must be in a subfolder! BpyBuild does not support using
  `.` for `addon_folder`
...
When we build, the default case will always run. We can also define cases we want
to only run if we specify them:
